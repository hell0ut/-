...
def shannon_fano_decode(enc_message, name, ext, codes_dict):
    ...
    new_codes_dict = None
    dict_len = None
    dict_len_binary = None
    encoded_message = ''

    i = 0
    while i < 2:
        dict_len_binary = f'{decoded_bytes[i]:b}'
        if i != 0:
            dict_len_binary += f'{decoded_bytes[i]:b}'
        dict_len = int(dict_len_binary, 2)
        i += 1
        print(dict_len_binary)


    print(f'{dict_len} after file')
...
def write_file(encoded_message, name, json_dict):
    bytes_array = []
    four_byte_length = []
    raw_len = f'{len(json_dict):b}'
    while len(raw_len) > 0:
        four_byte_length.append(int(raw_len[0:8], 2))
        raw_len = raw_len[8:]

    print('for byte', four_byte_length)
    bytes_s = b''
    ...
    print(bytes_s)
    print(f'len of dict befor file - {len(json_dict)}')


def serialize_dict(codes_dict):
    json_dict = json.dumps(codes_dict)
...
if __name__ == '__main__':
    ...
    write_file(message, file_name.split('.')[0], serialize_dict(sym_to_code_dict))
    print()
    message1, encoded_message = shannon_fano_decode('2', file_name.split('.')[0], file_name.split('.')[1], sym_to_code_dict)
    print(f'The shanone-fano code equality:{encoded_message == message}, len1 = {len(encoded_message)}, {len(message)}')
    serialize_dict(sym_to_code_dict)
